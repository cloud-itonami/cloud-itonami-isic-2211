(ns tyremfg.operation-test
  "Smoke tests for the compiled TyreOperationActor graph itself (build
  + one happy path per op). The governor's full rule contract (HARD
  holds, escalation, phase gating) is exercised in
  `tyremfg.governor-contract-test`; the Store contract in
  `tyremfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "TyreOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "builder-001" :severity :moderate :description "curing press temperature anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
