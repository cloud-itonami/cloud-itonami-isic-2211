(ns tyremfg.governor
  "Tyre Plant Operations Governor -- the independent compliance layer
  that earns the TyreAdvisor the right to commit. The advisor has no
  notion of whether a piece of equipment it wants to schedule
  ...
  tries to ACTUATE (rather than merely draft-schedule) the building/
  curing/vulcanization line, whether a proposal secretly tries to
  self-issue a DOT/ECE tyre-safety CERTIFICATION (an authority this
  actor never holds), whether a shipment proposal's own claimed
  ...
  becomes direct building/curing-line-equipment control, so this MUST
  be a separate system able to *reject* a proposal and fall back to
  HOLD.

  `:itonami.blueprint/governor` is `:tyre-plant-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       building/curing-line-equipment
    ...
    4. Line-actuate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       Directly actuating the building/
                                       curing/vulcanization line is this
                                       ...
                                       `tyremfg.phase`: this op is never
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       attempting to self-issue a DOT
                                       (US) / ECE (EU) tyre-safety
                                       certification mark -- an
   ...
   11. Invalid load-index          -- for `:log-production-batch`, if
                                       ...
                                       code (`tyremfg.registry/load-
                                       index-valid?`), the batch record
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct building/
...
(defn- line-control-blocked-violations
  ...
  Anything else (direct tyre-building/curing/vulcanization-line
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はタイヤ成型・加硫等の設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-actuate-blocked-violations
  ...
  directly actuate the tyre-building/curing/vulcanization line -- this
  actor may only ever propose/schedule a DRAFT maintenance window,
  never actuate the line directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-line? (:value proposal))))
    [{:rule :line-actuate-blocked
      :detail "成型/加硫ラインの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a DOT/ECE tyre-safety certification mark -- an
  authority exclusively reserved to the accredited certification body,
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "DOT/ECEタイヤ安全認証の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-tyre-category-violations
  ...
  let a fabricated tyre category through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [tyre-category (:tyre-category (:value proposal))]
      (when (and (some? tyre-category) (not (registry/tyre-category-valid? tyre-category)))
        [{:rule :invalid-tyre-category
          :detail (str tyre-category " は既知の tyre-category 値ではない")}]))))

(defn- invalid-load-index-violations
  "For `:log-production-batch`, if the patch declares a `:load-index`
  that is not a physically plausible ETRTO/DOT code, reject rather
  than let fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [li (:load-index (:value proposal))]
      (when (and (some? li) (not (registry/load-index-valid? li)))
        [{:rule :invalid-load-index
          :detail (str li " は物理的に妥当な load-index の範囲外")}]))))
...
(defn check
  "Censors a TyreAdvisor proposal against the governor rules.
