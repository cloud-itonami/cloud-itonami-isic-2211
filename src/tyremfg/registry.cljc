(ns tyremfg.registry
  "Pure-function domain logic for the rubber-tyre-and-tube plant-
  operations coordination actor -- equipment/batch verification,
  shipment-quantity recompute, tyre-category validation, load-index
  ...
  pre-existing `kotoba-lang/tyremfg`-style capability library to wrap
  (verified: no such repo exists). The domain logic therefore lives
  here as pure functions, re-verified INDEPENDENTLY by
  `tyremfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `resinmfg.registry/shipment-weight-exceeded?` from
  `cloud-itonami-isic-2013`, and `plasticsmfg.registry` from
  `cloud-itonami-isic-2220`): never trust a proposal's own
  ...
  coordinated shipment), not the act of actuating a building/curing/
  vulcanization line or dispatching a real freight carrier, and never
  the act of issuing a DOT/ECE tyre-safety certification (this actor
  NEVER does any of those -- see README `What this actor does NOT
  do`).

  SCOPE: ISIC 2211 covers rubber tyre and tube manufacturing --
  compounding, tyre-building (green-tyre assembly), curing/
  vulcanization, and finishing/inspection lines producing finished
  pneumatic and solid tyres and inner tubes. This actor coordinates the
  ...
  coordination) -- it never touches the building/curing/vulcanization
  equipment directly, and it never stands in for the certification body
  that issues DOT (US) / ECE (EU) tyre-safety approval marks.")

;; ----------------------------- constants -----------------------------

(def valid-tyre-categories
  "The closed set of tyre-category values a production-batch record may
  declare. Anything else is a fabricated/unrecognized tyre category --
  the governor HARD-holds rather than let an invented category pass
  through."
  #{:passenger :light-truck :truck-bus :agricultural
    :off-the-road :motorcycle :aircraft :industrial})

(def load-index-min
  "Physical floor for a tyre's own ETRTO/DOT load-index code (the
  standardized integer code that maps to a maximum load capacity
  table)."
  0)

(def load-index-max
  "Physical ceiling for a tyre's own ETRTO/DOT load-index code. A
  reading above this is an implausible/fabricated code, not a real
  standardized tyre rating."
  279)

(def defect-rate-min-percent
  "Physical floor for a batch's own curing/build defect-rate reading
  (zero defects is the best possible outcome, never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own curing/build defect-rate reading
  -- a batch cannot reject more than 100% of its own output. A reading
...
(defn batch-verified?
  ...
  (i.e. its tyre-category/load-index/quantity/defect-rate claims have
...
(defn tyre-category-valid?
  "Is `tyre-category` one of the closed, known tyre-category values?
  nil/blank is treated as invalid (a production-batch patch must
  declare a real tyre category, not omit it silently)."
  [tyre-category]
  (contains? valid-tyre-categories tyre-category))

(defn load-index-valid?
  "Is `load-index` a physically plausible ETRTO/DOT load-index code?
  Rejects nil, non-integers, negative values, and values beyond
  `load-index-max` -- a fabricated or sensor-error reading, never let
  through as a real tyre-rating fact."
  [load-index]
  (and (integer? load-index)
       (>= load-index load-index-min)
       (<= load-index load-index-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch curing/build defect-rate
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a DOT/ECE tyre-safety certification mark -- this
...
(defn register-maintenance
  ...
  building/curing-line maintenance window against a verified,
  registered piece of equipment. Pure function -- does not actuate the
  building/curing/vulcanization line or execute any maintenance; it
  builds the RECORD a plant coordinator would keep. `tyremfg.governor`
  ...
  the building/curing line (see README `Actuation`), before this is
...
(defn register-shipment
  ...
  outbound tyre shipment against a verified, registered production
  batch. Pure function -- does not dispatch any real freight carrier;
  it builds the RECORD a plant coordinator would keep. `tyremfg.governor`
  independently re-verifies the shipment's own claimed quantity against
  `shipment-quantity-exceeded?`, before this is ever allowed to commit."
