(ns tyremfg.operation
  "TyreOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (TyreAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Tyre Plant Operations
  ...
  (:require [langgraph.graph :as g]
            ...
            [tyremfg.advisor :as advisor]
            [tyremfg.governor :as governor]
            [tyremfg.phase :as phase]
...
(defn build
  "Compiles a TyreOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `tyremfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TyreAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Tyre Plant Operations Governor -- independent censor.
      (g/add-node :govern
