(ns tyremfg.advisor
  "TyreAdvisor -- the *contained intelligence node* for the rubber-
  tyre-and-tube plant-operations coordination actor.

  It normalizes production-batch patches (tyre-category/load-index/
  quantity/defect-rate), drafts a building/curing-line maintenance
  scheduling proposal against a piece of equipment, drafts a safety-
  concern flag, and drafts an outbound tyre shipment coordination
  ...
  NEVER a real building/curing-line actuation, freight dispatch, or
  DOT/ECE tyre-safety certification -- see README `What this actor
  does NOT do`. Every output is censored downstream by
  `tyremfg.governor` before anything touches the SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct building/
                                 ; curing-line-control effect
  ...
  contract every caller of this actor agrees to) -- `tyremfg.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [tyremfg.registry :as registry]
            [tyremfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's tyre-category,
  load-index, quantity, defect-rate or verification status. High
...
(defn- schedule-maintenance
  "Draft a building/curing-line maintenance-window scheduling proposal
  against a piece of equipment. The advisor reports what it can see
  (equipment verified?/registered?) in its rationale, but
  `tyremfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a curing-defect/DOT-compliance/equipment-safety concern.
  ...
  technicality). See `tyremfg.phase`: no phase ever adds this op to a
  phase's `:auto` set; `tyremfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound tyre shipment coordination proposal against a
  production batch. The advisor passes through the caller's own
  claimed unit quantity -- it does NOT invent one, and
  `tyremfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたはタイヤ・チューブ製造工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "成型・加硫等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "DOT/ECEタイヤ安全認証を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `tyremfg.governor`
