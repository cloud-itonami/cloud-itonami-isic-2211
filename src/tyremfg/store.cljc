(ns tyremfg.store
  "SSoT for the rubber-tyre-and-tube plant-operations coordination
  actor, behind a `Store` protocol so the backend is a swap, not a
  rewrite -- the same seam every `cloud-itonami-isic-*` actor in this
  fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-2013`'s own
  `resinmfg.store`, `cloud-itonami-isic-2220`'s own `plasticsmfg.store`),
  ...
  self-contained (no external tyre-manufacturing capability library,
  no jurisdiction-scoped Datomic-parity requirement driving a second
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             tyre-category/load-index/quantity/defect-
                             rate record. `:verified?` marks whether
                             the batch's own claims have actually been
                             QC-inspected (never inferred from a
                             routine intake patch); `:registered?`
                             marks whether it is on file in the
    ...
    - `maintenance`       -- a scheduled building/curing-line
                             maintenance-window DRAFT against a piece
                             of equipment (`tyremfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound tyre shipment DRAFT
                             (`tyremfg.registry`'s `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (tyremfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (tyremfg.registry drafts)")
...
(defn- sample-equipment []
  {"builder-001" {:id "builder-001" :kind :tyre-building-machine
                  :verified? true :registered? true
                  :last-maintenance-date "2026-05-01"}
   "curing-press-002" {:id "curing-press-002" :kind :curing-press
                        :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `tyremfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `tyremfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  tyre-building-machine unit (schedulable for maintenance), one
  UNVERIFIED/unregistered curing-press unit (blocks any maintenance
