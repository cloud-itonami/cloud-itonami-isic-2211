(ns tyremfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE the building/
  curing line directly (permanently blocked, no override), a proposal
  that tries to self-issue a DOT/ECE tyre-safety certification
  (permanently blocked, no override), a double-schedule of the same
  maintenance window, a production-batch patch with a fabricated tyre
  category, a production-batch patch with an implausible load-index,
  and a production-batch patch with an implausible defect-rate
  reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on builder-001 (verified, registered, tyre-building machine -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on builder-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:tyre-category :passenger}}
                       coordinator))

    (println "== unrecognized op -> HARD hold ==")
    (println (exec-op actor "t6"
                       {:op :actuate-curing-line :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on curing-press-002 (UNVERIFIED/unregistered curing press -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on builder-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated tyre-category -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:tyre-category :unobtainium}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible load-index -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:load-index 9999}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a DOT/ECE certification -> HARD hold, PERMANENT ==")
